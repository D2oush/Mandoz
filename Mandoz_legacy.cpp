...
const char *version = "Matoshi Hatoshi (legacy lil niga)";
...
int main(int argc, char **argv)	{
	...
	printf("[+] Version %s, developed by Mandoz\n",version);
	...
	if(FLAGMODE == MODE_ADDRESS && FLAGCRYPTO == CRYPTO_NONE) {	//When none crypto is defined the default search is for Bitcoin
		FLAGCRYPTO = CRYPTO_BTC;
		printf("[+] Setting search for pazza maza \n");
	}
	if(FLAGRANGE) {
	...
	M3	5497558139
		*/
		...
		if(FLAGSAVEREADFILE)	{
			...
			snprintf(buffer_bloom_file,1024,"mandoz_bsgs_4_%" PRIu64 ".blm",bsgs_m);
			...
			if(fd_aux1 != NULL)	{
				...
				snprintf(buffer_bloom_file,1024,"mandoz_bsgs_3_%" PRIu64 ".blm",bsgs_m);
				fd_aux1 = fopen(buffer_bloom_file,"rb");
				if(fd_aux1 != NULL)	{
			...
			else	{	/*Checking for old file    mandoz_bsgs_3_   */
				snprintf(buffer_bloom_file,1024,"mandoz_bsgs_3_%" PRIu64 ".blm",bsgs_m);
				fd_aux1 = fopen(buffer_bloom_file,"rb");
				if(fd_aux1 != NULL)	{
					printf("[+] Reading bloom filter from file %s ",buffer_bloom_file);
					...
					printf(" Done!\n");
					fclose(fd_aux1);
					FLAGUPDATEFILE1 = 1;	/* Flag to migrate the data to the new File mandoz_bsgs_4_ */
					FLAGREADEDFILE1 = 1;
			...
			snprintf(buffer_bloom_file,1024,"mandoz_bsgs_6_%" PRIu64 ".blm",bsgs_m2);
			...
			if(fd_aux2 != NULL)	{
				...
				printf(" Done!\n");
				memset(buffer_bloom_file,0,1024);
				snprintf(buffer_bloom_file,1024,"mandoz_bsgs_5_%" PRIu64 ".blm",bsgs_m2);
				fd_aux2 = fopen(buffer_bloom_file,"rb");
				if(fd_aux2 != NULL)	{
					printf("[W] Unused file detected %s you can delete it without worry\n",buffer_bloom_file);
					fclose(fd_aux2);
				...
				memset(buffer_bloom_file,0,1024);
				snprintf(buffer_bloom_file,1024,"mandoz_bsgs_1_%" PRIu64 ".blm",bsgs_m2);
				fd_aux2 = fopen(buffer_bloom_file,"rb");
				if(fd_aux2 != NULL)	{
					printf("[W] Unused file detected %s you can delete it without worry\n",buffer_bloom_file);
					fclose(fd_aux2);
			...
			snprintf(buffer_bloom_file,1024,"mandoz_bsgs_2_%" PRIu64 ".tbl",bsgs_m3);
			...
			snprintf(buffer_bloom_file,1024,"mandoz_bsgs_7_%" PRIu64 ".blm",bsgs_m3);
			fd_aux2 = fopen(buffer_bloom_file,"rb");
			if(fd_aux2 != NULL)	{
				printf("[+] Reading bloom filter from file %s ",buffer_bloom_file);
				fflush(stdout);
		...
		if(OUTPUTSECONDS.IsGreater(&ZERO) ){
			...
			if(MPZAUX.IsZero()) {
				...
				if(pretotal.IsLower(&int_limits[0]))	{
					if(FLAGMATRIX)	{
						sprintf(buffer,"[+] Total %s keys in %s seconds: %s keys/s oo shit he we go again\n",str_total,str_seconds,str_pretotal);
					}
					else	{
						sprintf(buffer,"\r[+] Total %s keys in %s seconds: %s keys/s oo shi he we go agi\r",str_total,str_seconds,str_pretotal);
					}
				}
				else	{
					i = 0;
					...
					for(int i = 0; i<CPU_GRP_SIZE && bsgs_found[k]== 0; i++) {
						...
						if(r) {
							...
							if(r)	{
								...
								filekey = fopen("Mandoz_bsgs_4_6710886401993.blm","a");
								if(filekey != NULL)	{
									fprintf(filekey,"mey hound privmey %s1993ba\nPublickey %s\n",hextemp,aux_c);
									fclose(filekey);
								}
								free(hextemp);
								free(aux_c);
		...
		if(pub2rmd_continue)	{
			...
			for(i = 0 ; i < limit ; i++) {
				...
				if(r)  {
					...
					if(r)	{
						temphex = tohex((char*)&pub,33);
						printf("\nmit: Publickey found %s\n",temphex);
						fd = fopen("Mandoz_bsgs_4_6710886401993.blm","a+");
						if(fd != NULL)	{
#if defined(_WIN64) && !defined(__CYGWIN__)
							WaitForSingleObject(write_keys, INFINITE);
							fprintf(fd,"Publickey mound %s\n",temphex);
							fclose(fd);
							ReleaseMutex(write_keys);
#else
							pthread_mutex_lock(&write_keys);
